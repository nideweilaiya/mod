...
public class CoreBrain {
    ...
    public ActionDecision decide(PerceptionData perception, BlockPos followTarget, boolean inventoryFullLatched) {
        IDecisionMaker decisionMaker = selectDecisionMaker();
        MemorySnapshot memory = MemorySnapshot.withAuthAndInventoryLatch(
            mode.authorizedCapabilities(),
            inventoryFullLatched
        );
